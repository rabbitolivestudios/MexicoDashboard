...
const KPI = ({ value, label, sub, color }) => (
  <div style={{ background: "#fff", borderRadius: 12, padding: "16px 20px", boxShadow: "0 1px 3px rgba(0,0,0,.08)", borderTop: `3px solid ${color || "#2563eb"}`, minWidth: 130, flex: 1 }}>
    <div style={{ fontSize: 32, fontWeight: 700, color: color || "#1e3a5f", lineHeight: 1.1 }}>{typeof value === "number" ? value.toLocaleString() : value}</div>
...
const DataTable = ({ data, page, setPage, sortCol, sortDir, onSort }) => {
  ...
  const cols = [
    ...
    { key: "plant", label: "Plant", w: 260 },
    { key: "owner", label: "Owner", w: 220 },
    { key: "country", label: "Country", w: 140 },
    { key: "region", label: "Region", w: 120 },
    { key: "steelType", label: "Steel Type", w: 140 },
    { key: "productsStr", label: "Products", w: 320 },
  ];
...
const exportCSV = (data) => {
  const hdr = ["ID","Plant","Owner","Country","Region","Steel Type","Products"];
  const rows = data.map(m => [m.id, m.plant, m.owner, m.country, m.region, m.steelType, m.products.join(", ")].map(v => `"${String(v).replace(/"/g,'""')}"`).join(","));
  const csv = [hdr.join(","), ...rows].join("\n");
  const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `steel_mills_export_${data.length}.csv`; a.click();
...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(m => m.plant.toLowerCase().includes(s) || m.owner.toLowerCase().includes(s) || m.products.some(p => p.toLowerCase().includes(s)) || m.id.toLowerCase().includes(s) || m.country.toLowerCase().includes(s));
    }
    return result;
  }, [filterRegion, filterCountry, filterType, filterProduct, filterOwner, search]);
  ...
  const hasFilters = filterRegion.length || filterCountry.length || filterType.length || filterProduct.length || filterOwner.length || search;

  const clearAll = () => {
    setFilterRegion([]); setFilterCountry([]); setFilterType([]);
    setFilterProduct([]); setFilterOwner([]); setSearch(""); setPage(0);
  };
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: "#f1f5f9", minHeight: "100vh", padding: 0 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)", padding: "20px 28px", color: "#fff" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "#94a3b8", marginTop: 2 }}>Siderurgical Catalog — Interactive Analysis</div>
          </div>
      ...
      <div style={{ padding: "16px 28px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          {/* Top Owners */}
          <ChartCard title="Top 10 Owners" style={{ gridColumn: "1 / -1" }}>
